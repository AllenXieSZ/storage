...
## 背景（先读）
- 小帅 2026-09-06 04:30 UTC 指令：**"这个资源删除，然后再重新测一测。安排那个子agent去测试，之前配置的那个子agent，一样画图，把两次测试总结推送到 github。"**
...
7. **清理**：RUN2 测完 **默认保留**新资源（小帅习惯），报告注明新资源 ID + 清理命令。删前问小帅。
...
- 私网挂载/取数据走 SSM + S3 中转。
- 严禁手写伪 tool call。
- 起真实资源前若需报预算：本次约 $3-6 量级（1 个 1536 档 FSxN 跑 ~1h + 一次升吞吐），小帅本条指令已明确"重新测"= 已预批，直接执行不必停等。
