...
class Cart {
	...
	public function __construct(\Opencart\System\Engine\Registry $registry) {
		...
		// [CART_REDIS] 购物车条目存储后端（redis 或 db），减少 Aurora 压力
		$this->storage = new \Opencart\System\Library\Cart\CartStorage($registry);

		if ($this->storage->usingRedis()) {
			// Redis 引擎：过期由 key TTL 自动处理，无需 DELETE 过期车
			if ($this->customer->isLogged()) {
				// 登录时把匿名购物车合并到登录用户名下
				$this->storage->mergeOnLogin((int)$this->config->get('config_store_id'), (int)$this->customer->getId(), $this->session->getId());
			}
		} else {
	...
	public function getProducts(): array {
		if (!$this->data) {
			// [CART_REDIS] 从 Redis 或 DB 读购物车条目
			if ($this->storage->usingRedis()) {
				$cart_rows = $this->storage->getItems((int)$this->config->get('config_store_id'), (int)$this->customer->getId(), $this->session->getId());
			} else {
	...
	public function add(int $product_id, int $quantity = 1, array $option = [], int $subscription_plan_id = 0, array $override = []): void {
		// [CART_REDIS]
		if ($this->storage->usingRedis()) {
			$this->storage->addItem((int)$this->config->get('config_store_id'), (int)$this->customer->getId(), $this->session->getId(), (int)$product_id, (int)$quantity, json_encode($option), (int)$subscription_plan_id, json_encode($override));
			$this->data = [];
			return;
		}
	...
	public function update(int $cart_id, int $quantity): void {
		// [CART_REDIS]
		if ($this->storage->usingRedis()) {
			$this->storage->updateItem((int)$this->config->get('config_store_id'), (int)$this->customer->getId(), $this->session->getId(), (int)$cart_id, (int)$quantity);
			$this->data = [];
			return;
		}
	...
	public function remove(int $cart_id): void {
		// [CART_REDIS]
		if ($this->storage->usingRedis()) {
			$this->storage->removeItem((int)$this->config->get('config_store_id'), (int)$this->customer->getId(), $this->session->getId(), (int)$cart_id);
	...
	public function clear(): void {
		// [CART_REDIS]
		if ($this->storage->usingRedis()) {
			$this->storage->clearItems((int)$this->config->get('config_store_id'), (int)$this->customer->getId(), $this->session->getId());
			$this->data = [];
			return;
		}
...
// ===== [CART_REDIS] 内联购物车 Redis 存储后端（避免 autoload 问题）=====
class CartStorage {
	private $engine = 'db';
	private $redis = null;
	private $prefix = '';
