...
| 2026-08-24 | 1.4728 | 26.3417 | — | 24.8689 | $0.5222 | $0 | 取到 08-23 新点；公式 $0.52/月 vs CE $0（CE 24–48h 延迟 + 极小额未入账，待回填） |
| 2026-08-25 | 1.4728 | 27.8145 | — | 26.3417 | $0.5532 | $0 | 取到 08-24 新点；公式 $0.55/月 vs CE $0（CE 24–48h 延迟 + 极小额未入账，待回填） |
| 2026-08-26 | 1.4728 | 30.7600 | — | 29.2872 | $0.6150 | $0 | 取到 08-25 新点；单日跳增 +2.95 GB（≈2 轮 DML，窗口边界波动）；公式 $0.62/月 vs CE $0（CE 24–48h 延迟 + 极小额未入账，待回填） |
| _(待采集)_ | | | | | | | |
...
> **2026-08-24 采集简评（取到 08-23 新点，日增回落到 +1.47 GB，补点假设获印证）**：本次采集取到了 **08-23 新发布的备份数据点**（此前 08-23 采集时因 06:30 UTC 早于日更点 08:30 UTC 而沿用了 08-22 值）。`BackupRetentionPeriodStorageUsed` 从 08-22 的 24.8689 GB 涨到 **26.3417 GB**（单日 **+1.473 GB**），日增**回落到近日稳定的 ~1 轮 DML 备份增量量级**——**印证了 08-23 简评提出的"补点假设"**：08-22 那次 +2.95 GB 的单日跳增确属 CloudWatch 发布节奏波动导致的一次性两日增量补点，而非计费量真实斜率上升；扣除补点后底层日增仍稳定在 +1.47 GB/天。`TotalBackupStorageBilled` 从 23.3961 GB 涨到 **24.8689 GB**（+1.473 GB，**再次精确等于前一采集日的 Retention 值 24.8689 GB**，"Billed 相对 Retention 恰好滞后约 1 天"的规律在 08-22 短暂打破后**本日恢复**）。计费量 `TotalBackupStorageBilled = max(0, 26.3417 − 1.4728) = 24.8689 GB`（脚本取 CloudWatch 值，与手算一致）。按公式 **月度费用 = 24.8689 × $0.021 ≈ $0.5222/月**，较 08-22 的 $0.4913/月增约 6%（增幅回落到 ~6%/天，符合日增回归 +1.47 GB 后的线性爬升；**费用已首次突破 $0.50/月**）。`VolumeBytesUsed` 持平于 **1.4728 GB**（高水位完全收敛，连续第 9 天无变化）。CE 实际费用仍显示 **$0**——延续 08-23 核实结论：本测试规模（月费 ~$0.52）下 CE 全程 $0，连续 17 天（08-08 起）非零计费量未在 CE 反映，公式 vs 实测最终核对以 **9 月初 8 月完整账单**为准。`SnapshotStorageUsed` **仍为空**（延续既往结论：AWS Backup 托管 weekly snapshot 走 Backup 服务自身计量，不反映在 RDS 集群维度指标上）。**运维提示**：本次 cron 正常执行（08-24 06:30 UTC）。**趋势判断**：`BackupRetentionPeriodStorageUsed` 仍以 +1.47 GB/天稳定爬升（24.9→26.3 GB），尚未进入平台期——每日 20GB DML 持续下，7 天窗口稳态计费量或继续攀升；备份计费量已达 ~24.9 GB（公式 ~$0.52/月）。
>
> **2026-08-25 采集简评（取到 08-24 新点，日增稳定 +1.47 GB，线性爬升延续）**：本次采集取到了 **08-24 新发布的备份数据点**。`BackupRetentionPeriodStorageUsed` 从 08-23 的 26.3417 GB 涨到 **27.8145 GB**（单日 **+1.473 GB**），日增**继续稳定在 ~1 轮 DML 备份增量量级**（连续第二天 +1.47 GB，进一步确认 08-22 的 +2.95 GB 为一次性补点、底层日增稳态为 +1.47 GB/天）。`TotalBackupStorageBilled` 从 24.8689 GB 涨到 **26.3417 GB**（+1.473 GB，**再次精确等于前一采集日的 Retention 值 26.3417 GB**，"Billed 相对 Retention 恰好滞后约 1 天"的规律**连续第二天保持**）。计费量 `TotalBackupStorageBilled = max(0, 27.8145 − 1.4728) = 26.3417 GB`（脚本取 CloudWatch 值，与手算一致）。按公式 **月度费用 = 26.3417 × $0.021 ≈ $0.5532/月**，较 08-24 的 $0.5222/月增约 6%（增幅稳定在 ~6%/天，符合 +1.47 GB/天日增下的线性爬升）。`VolumeBytesUsed` 持平于 **1.4728 GB**（高水位完全收敛，连续第 10 天无变化）。CE 实际费用仍显示 **$0**——延续既往结论：本测试规模（月费 ~$0.55）下 CE 全程 $0，连续 18 天（08-08 起）非零计费量未在 CE 反映，公式 vs 实测最终核对以 **9 月初 8 月完整账单**为准（已进入 8 月尾声，账单核对窗口临近）。`SnapshotStorageUsed` **仍为空**（延续既往结论：AWS Backup 托管 weekly snapshot 走 Backup 服务自身计量，不反映在 RDS 集群维度指标上）。**运维提示**：本次 cron 正常执行（08-25 06:30 UTC）。**趋势判断**：`BackupRetentionPeriodStorageUsed` 仍以稳定 +1.47 GB/天线性爬升（26.3→27.8 GB），尚未进入平台期；每日 20GB DML 持续下，7 天窗口稳态计费量继续攀升，备份计费量已达 ~26.3 GB（公式 ~$0.55/月）。
>
> **2026-08-26 采集简评（取到 08-25 新点，单日跳增 +2.95 GB，线性爬升延续）**：本次采集取到了 **08-25 新发布的备份数据点**。`BackupRetentionPeriodStorageUsed` 从 08-24 的 27.8145 GB 涨到 **30.7600 GB**（单日 **+2.946 GB**）——本次单日增量回到 ~2 轮 DML 备份增量的量级，较前两日稳定的 +1.47 GB/天偏高，属 7 天窗口边界老 change records 滚出量与新增量逐日错位的正常波动（历史上 08-14、08-18、08-22 均出现过类似 +2.9 GB 的跳增点，与 +1.47 GB/天交替出现，两日平均仍约 +2.2 GB/天）。`TotalBackupStorageBilled` 从 26.3417 GB 涨到 **29.2872 GB**（+2.946 GB，本次不再精确等于前一采集日的 Retention 值 27.8145 GB——比它高约 1.47 GB，与 08-18 那次一样说明 Billed 相对 Retention 的发布滞后在跳增日会摆动到 0~1 天之间，非严格固定）。计费量 `TotalBackupStorageBilled = max(0, 30.7600 − 1.4728) = 29.2872 GB`（脚本取 CloudWatch 值，与手算一致）。按公式 **月度费用 = 29.2872 × $0.021 ≈ $0.6150/月**，较 08-25 的 $0.5532/月增约 11%（增幅较前两日的 ~6% 回升，对应本次 Retention 单日 +2.95 GB 的较大增量）。`VolumeBytesUsed` 持平于 **1.4728 GB**（高水位完全收敛，连续第 11 天无变化）。CE 实际费用仍显示 **$0**——延续既往结论：本测试规模（月费 ~$0.62）下 CE 全程 $0，连续 19 天（08-08 起）非零计费量未在 CE 反映，公式 vs 实测最终核对以 **9 月初 8 月完整账单**为准（已进入 8 月最后一周，账单核对窗口临近）。`SnapshotStorageUsed` **仍为空**（延续既往结论：AWS Backup 托管 weekly snapshot 走 Backup 服务自身计量，不反映在 RDS 集群维度指标上）。**运维提示**：本次 cron 正常执行（08-26 06:30 UTC）。**趋势判断**：`BackupRetentionPeriodStorageUsed` 仍以线性爬升（27.8→30.8 GB，日均 +1.5~2.9 GB 摆动），尚未进入平台期；每日 20GB DML 持续下，7 天窗口稳态计费量继续攀升，备份计费量已突破 ~29 GB（公式 ~$0.62/月）。
