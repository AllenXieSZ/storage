...
多进程并发扫描后批量写入 CSV。可选只取文件名（更快），或同时取 名字/大小/时间。
...
| 规模 | 目录数 | 并发 | 名字+大小+时间 | 只取文件名 |
|---|---|---|---|---|
| 10 万文件 | 781 | 64 | 16 s | **2.3 s** |
| 100 万文件 | 9,331 | 64 | 182 s（约 3 分钟） | **25 s** |

只取文件名约快 7 倍（纯 MDT 查询，不额外取文件属性）。
...
# 只取文件名（更快）
python3 lustre_ls_csv_parallel.py -d /mnt/lustre/many1m -o out.csv -j 64 --names-only
```
CSV 列：
- 完整模式：`name,dir,path,size_bytes,size_human,mtime_epoch,mtime_iso`
- `--names-only`：`name,dir,path`
