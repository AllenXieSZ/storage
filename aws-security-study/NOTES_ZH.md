...
**Macie是什么**：用机器学习自动发现、分类、保护S3里敏感数据的服务。核心能力：
- **敏感数据发现**：扫S3桶自动识别PII(个人身份信息)、财务数据、凭证/密钥、健康信息(PHI)等。
- **S3安全态势评估**：持续评估S3桶哪些公开可访问/未加密/被外部账号共享,出清单告警。
- **数据分类**：按敏感类型打标+生成报告,满足GDPR/HIPAA/PCI合规("敏感数据有没有放错地方")。
- **发现汇总**：findings送Security Hub/EventBridge接自动化响应。

**纵深防御六层位置**：数据层+检测层交叉。数据层角度=关心"敏感数据在哪、有没有被保护"(与KMS加密"锁住数据"角度不同,Macie是"先找出哪些数据需重点锁");检测层角度=持续扫描+出findings+汇总Security Hub(路数同GuardDuty/Inspector)。

**与易混服务区别(重点)**：
| 服务 | 管什么 | 一句话 |
|---|---|---|
| Macie | S3敏感数据发现/分类 | 找"哪些数据敏感、放哪、有没暴露" |
| GuardDuty | 威胁检测(可疑行为) | 找"有没有人在攻击"(Q21) |
| Inspector | 漏洞扫描(CVE) | 找"系统有没有已知漏洞"(Q26) |
| Config | 配置合规 | 找"资源配置合不合规"(Q20) |
| KMS/加密 | 数据加密 | "锁住"数据(Q14-17) |
一句话区分：**Macie=数据分类与敏感数据风险发现,回答"我有哪些敏感数据、在不在该在的地方、有没有被保护",别的服务不干这个。**

**AWS↔GCP对照**：敏感数据发现/分类=Macie↔GCP **Sensitive Data Protection(原Cloud DLP)**(扫描识别分类脱敏PII等)。哲学一致=加密保护之外,先知道哪些是敏感数据、在哪里,才能针对性保护。
