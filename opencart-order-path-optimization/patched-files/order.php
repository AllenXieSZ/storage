...
class Order extends \Opencart\System\Engine\Model {
	...
	public function addOrder(array $data): int {
		...
		// Products (batch order_product INSERT + option 关联回填)
		...
		// Totals (批量单条多值 INSERT)
		if (!empty($data['totals'])) {
			$rows = [];
			foreach ($data['totals'] as $total) {
				$rows[] = "('" . (int)$order_id . "', '" . $this->db->escape($total['extension']) . "', '" . $this->db->escape($total['code']) . "', '" . $this->db->escape($total['title']) . "', '" . (float)$total['value'] . "', '" . (int)$total['sort_order'] . "')";
			}
			$this->db->query("INSERT INTO `" . DB_PREFIX . "order_total` (`order_id`, `extension`, `code`, `title`, `value`, `sort_order`) VALUES " . implode(', ', $rows));
		}
