# FSx for Lustre 容量扩容耗时实测报告：1.2 TiB → 2.4 TiB（含灌数据量对照）

> ⚠️ **仅本次测试环境实测，不代表官方结论。** 单次实测，样本 n=1，实际耗时受容量、已用数据量、DeploymentType、区域负载等影响。
>
> 📊 **本报告含两组对照实验**（唯一变量=灌数据量）：
> - **测试 A**：灌 ~900 GB（79% 满）
> - **测试 B**：灌 ~1.05 TB（92% 满）
> 详见下方「⭐ 灌数据量对照（A vs B）」章节。

## 结论先行

**在本次测试环境下，FSx for Lustre（PERSISTENT_2 / 500 MB/s/TiB）把容量从 1.2 TiB 扩到 2.4 TiB，从发起 `update-file-system` 到文件系统 Lifecycle 回到 `AVAILABLE`（新容量 2400 GiB 可用）：**
- **测试 A（灌 900G / 79% 满）** ≈ **15.7 分钟（943 秒）**
- **测试 B（灌 1.05TB / 92% 满）** ≈ **16.8 分钟（1007 秒）**
- → **接近容量满，"到 AVAILABLE" 几乎不变（+1.1 分，+7%）；真正变长的是后台重分布（+18%）。**

- 之后还有一段**后台 `STORAGE_OPTIMIZATION`（数据重分布）**，把老数据从原 OST 均衡到新增 OST，**不阻塞使用**（此时 Lifecycle 已是 AVAILABLE、文件系统可正常读写）。该重分布从发起 update 起算：**测试 A ≈ 38.9 分钟（2336 秒）；测试 B ≈ 46.0 分钟（2763 秒）**。
- **扩容机制 = 加 OST**：1.2 TiB 时 1 个 OST；扩到 2.4 TiB 后变成 2 个 OST（容量翻倍靠新增 OST0001），并非把原 OST 撑大。MDT 容量也从 34.4G 涨到 69.0G。两组测试机制一致。

---

## ⭐ 灌数据量对照（测试 A vs 测试 B）

**唯一变量 = 灌数据量**，其余参数（PERSISTENT_2 / 500 MB/s/TiB / 1200→2400 GiB / us-east-2c / c6in.2xlarge）完全一致。

| 指标 | 测试 A（灌 900G / 79% 满）| 测试 B（灌 ~1.05TB / 92% 满）| 差值 | 方向 |
|---|---|---|---|---|
...
| AVAILABLE 时 OST 数 | 1→2 (新增 OST0001) | 1→2 (新增 OST0001) | 同 | 机制一致 |
| 重分布后分布 | 549G : 361G (60:40) | 570G : 514G (53:47) | B 更均衡 | 数据越多越接近 50:50 |
| MDT 容量变化 | 34.4G→69.0G | 34.4G→69.0G | 同 | 机制一致 |

**对照结论（n=1，单次实测，谨慎）**：
1. **「到 AVAILABLE」几乎不随数据量变长**（+7%，落在轮询误差量级）——这段是扩容控制面/加 OST 操作，与已用数据量弱相关。**业务侧真正的「可用等待」两组都是 ~16 分钟。**
2. **「后台重分布」明显随数据量变长**（+18%）——要迁到新 OST 的老数据从 A 的 ~361G 增到 B 的 ~514G，搬得越多越久。这段**不阻塞使用**。
3. **数据越满，重分布后越接近 50:50**：B 达 570:514（53:47），比 A 的 549:361（60:40）更均衡。

**测试 B 的完整时间线 / OST 分布 / 复现命令**见附录：`REPORT_B_fill92.md`。
**附带修正**：AL2023（kernel 6.18）**默认 repo 不含 lustre-client 用户态工具**（`lfs`/`mount.lustre` 缺失，仅内核模块内置），需从 AWS FSx repo 手动安装。（本报告下文测试 A 早期"默认已含"的说法有误，以此为准。）

---

## 测试 A 详细数据（灌 900G / 79% 满）
