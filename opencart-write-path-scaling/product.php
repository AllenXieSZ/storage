...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProduct(int $product_id): array {
		// [PD-CACHE] 商品详情读缓存。OpenCart cache->get() 未命中返回 [](空数组),故用 empty() 判定未命中。
		$cache_key = 'product.detail.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . $product_id;
		$cached = $this->cache->get($cache_key);
		if (!empty($cached) && !isset($cached['__miss__'])) {
			return $cached;
		}
		...
		if ($query->num_rows) {
			...
			$this->cache->set($cache_key, $product_data);   // 回填缓存(仅缓存存在的商品)

			return $product_data;
		} else {
			return [];   // 不存在的商品不缓存(空数组无法与"未命中"区分,避免误判)
		}
	}
	...
	public function getOptions(int $product_id): array {
		// [PD-CACHE] 商品选项读缓存。未命中/无选项均返回 [](去查DB,选项数据小,可接受)
		$cache_key = 'product.options.' . (int)$this->config->get('config_language_id') . '.' . $product_id;
		$cached = $this->cache->get($cache_key);
		if (!empty($cached)) {
			return $cached;
		}
		...
		if ($product_option_data) {
			$this->cache->set($cache_key, $product_option_data);   // 仅缓存有选项的
		}
