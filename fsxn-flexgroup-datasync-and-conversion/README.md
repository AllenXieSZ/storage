...
> - 📸 **Snapshots do NOT block conversion (single-variable test, 2026-09-01)**: a **backup-free** clean FlexVol carrying 4 manual snapshots converts fine — `-check-only` returns **only warnings, no error** ("Conversion ... **can proceed** with the following warnings ..."), the live conversion returns `Job succeeded`, and the volume becomes `flexgroup`. Side effect (non-blocking): all pre-existing snapshots are set to **`pre-conversion`** state and can no longer be restored. **So: snapshot → convertible (warning only); backup → NOT convertible (Error, delete backup first). The only blocking variable is FSx native Backup, not snapshots.** See [`snapshot-vs-backup-block/`](../snapshot-vs-backup-block/).
